class Rom():
    def __init__(self, _rom_path=None):

        self.rom_path = _rom_path

        self.prg_rom = None
        self.chr_rom = None
        self.rom_banks = None
        self.vrom_banks = None
        self.mapper_number = None
        ...
        try:
            ...
            with open(self.rom_path, 'rb') as rom_file:
                ...
                if (header[0:4] != b'NES\x1A'):
                    raise ValueError("\nIt is not a valid NES header. Expected first four header values: b'NES\\x1A'. Found first four values: %s" % header[0:4])
                ...
                # Joining the higher with the lower bits (self.mapper_number = alfa | beta)
                self.mapper_number = ((header[6] >> 4) & 0xf) | (header[7] & 0xf0)

                # If there is a battery in the cartridge,
                # then there is a extended cpu ram
                if header[6] & 0x2: # 0x2 = 10b
                    self.extended_ram_exists = True
                    print("ROM uses extended RAM")
                else:
                    print("ROM does not use extended RAM")

                if (header[6] & 0x4):
                    raise ValueError("ROM uses trainer, but this functionality is not supported.")

                # Reads all 16kB PRG-ROM banks
                self.prg_rom = rom_file.read(0x4000 * self.rom_banks)
                if not self.prg_rom:
                    raise ValueEror("The number of PRG-ROM banks is valid. Should be greater than 0.")

                # Reads all 8kB CHR-ROM banks
                if self.vrom_banks:
                    self.chr_rom = rom_file.read(0x2000 * self.rom_banks)

                    print("Rom uses CHR-ROM")
                else:
                    print("Rom uses CHR-RAM")
